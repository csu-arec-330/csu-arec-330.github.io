...
## Cluster Summary Stats {.scrollable}

```{r}
#| echo: true
#| include: false
sumstats <- final_dataset %>%
  group_by(cluster) %>%
  summarise(
...
## Cluster 1: Fuel-Focused, Low-Volume Stores {.scrollable}

**These appear to be fuel-dominant convenience stores with very limited in-store purchases. Possibly rural or highway locations.**

- Few customers (~1.94 avg.)

- Very low sales (~$267 total)

- Very limited product diversity (~1.5 products)

- Almost all transactions are fuel (fuel share ~1.01)

- Small to mid-size chains (avg. chain size ~16.5)


## Cluster 2: Low-Volume, Non-Fuel Stores {.scrollable}

**These are likely small-format, low-traffic stores that don’t rely heavily on fuel. Possibly struggling or underutilized stores.**

- Fewest customers (~1.27)

- Lowest sales (~$21 total)

- Slightly more product diversity (~2.5 products)

- Very low fuel share (~7%)

- Mid-size chains (avg. chain size ~27)

## Cluster 3: High-Volume, High-Diversity Stores {.scrollable}

**These stores appear to be high-performing, full-service stores, possibly flagship locations or urban/high-traffic sites. Strong sales, product range, and large customer base.**

- Most customers (~11.66)

- Highest sales (~$2,855)

- High product diversity (~7.5 products)

- High fuel share (~91%)

- Large chains (avg. chain size ~105)
...
- The clusters suggest three distinct operational models among stores in the dataset
- These segments differ not only in customer base and sales

Why it matters:

- Product suppliers may target Cluster 3 with broader assortments.
- Clusters 1 and 2 may benefit from operational support or format redesign.
