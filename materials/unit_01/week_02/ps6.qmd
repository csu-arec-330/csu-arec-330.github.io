...
3. Use R to create a 3 year forecast for *each* chosen time series. Follow the steps we went through in lab.  
...
4. Create a dataframe for *each* time series using `bind_rows`, then write them to a csv file. You will end up with three csv files with the following variables: `measure_date`, `price`, `sa_price`, `trend`, `seasonal`, `random`, `forecast`, `upper`, `lower`. 
