...
The `forecast()` function returns **confidence intervals as matrices**, with each column representing different confidence levels (e.g., 80%, 95%). The `[,1]` indexing extracts the **first column**, which typically corresponds to the **80% confidence interval**.

#### Breakdown:
- `forecast_object$upper` and `forecast_object$lower` are **matrices**, not vectors.
- **First column (`[,1]`)** $\rightarrow$ Usually the **80% confidence interval**.
- **Second column (`[,2]`)** $\rightarrow$ Typically the **95% confidence interval** (if available).

#### Example:
```{r}
#| eval: false

carrot_forecast_upper[,1]  # Extracts 80% confidence level upper bound
carrot_forecast_lower[,1]  # Extracts 80% confidence level lower bound
```
:::
...
2. Make sure forecasted values (`forecast` = TRUE) are **filtered out** when generating the line graph. 
  - Drag `forecast` into the Filters card. Check the box next to True. Click OK. 
...
2. Create a new calculated field called `forecasted_price`.
  - `IF [Forecast]=TRUE THEN [Price] ELSE NULL END`
  
3. Plot the both observed and forecasted prices as a line chart. 
  - Add `measure_date` to the column shelf. Display `measure_date` as Measure > MONTH().
  - Add `observed_price` to the row shelf. Display as AVG(observed_price). 
  - Add `forecasted_price` to the row shelf. Display as AVG(forecasted_price). 
  - Right click AVG(forecasted_price) and select **Dual Axis**.

4. Plot the raw price data as a line chart. 
  - Add `observed_price` to the row shelf. Display as AVG(observed_price).
  
5. Use the second (stand-alone) observed price data (`observed_price`) to create a new forecast in Tableau.  
...
4. **Compare Tableau's forecast with R's forecast**:  
