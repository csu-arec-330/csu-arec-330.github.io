...
super_2018 <- read_csv("your_url_here.csv")
super_2019 <- read_csv("your_url_here.csv")
```
...
# Example: Replace missing values in a specific column
super_sales_all <- super_sales_all %>%
  mutate(column_name = replace_na(column_name, "Default Value"))
```
...
- Specify the dataframe you want to export (e.g., `super_sales_all`).

- Choose an appropriate filename (e.g., `"supermarket_sales_cleaned.csv"`). Check that your punctuation matches the example code!
