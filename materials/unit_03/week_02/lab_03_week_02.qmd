...
$$ StoreCount_i = \beta_0 + \beta_1 ~ MedianIncome_i + \beta_2 ~ AverageTemperature_i + \varepsilon_i $$
...
# Create a dataframe that combines hhi and pop
demo <- hhi %>%
  left_join(pop, by = "geoid")
...
county_demo <- county_store_map %>%
  select(-geoid.y) %>%
  rename(geoid = geoid.x) %>%
  inner_join(demo, by = "geoid") %>% # Join on county identifies (GEOID)
